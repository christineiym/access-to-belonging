...
export default function ChutesLaddersPlay({ numPlayers, bot, edgeLength }) {
    ...
    useEffect(() => {
        ...
        setAllPositions(p);
    }, [startCornerLeft, startCornerBottom, isRows]);  // removed edgeLength
    ...
    useEffect(() => {
        ...
        setMovers(m);
    }, [boardResetKey]);  // removed edgeLength
    ...
    useEffect(() => {
        ...
        console.log("start", playerPositions, currentPlayerIndex);

        const moveValue = lastScenarioDraw.moveValue ?? Number(lastScenarioDraw); // allow num or object
        const actualPlayerIndex = currentPlayerIndex >= 0 ? currentPlayerIndex : 0;
        const player = playerPositions.length <= 0 ? playersAtStart[actualPlayerIndex] : playerPositions[actualPlayerIndex];
    ...
    useEffect(() => {
        if (turnInProgress) {
            ...
            let playerName;
            if (playerPositions.length > 0) {
                playerName = playerPositions[currentPlayerIndex]["name"];
            } else {
                playerName = playersAtStart[currentPlayerIndex]["name"];
            }

            // Only run if there is a path left to run; reset path upon completion
            if (animationStepIndex < currentPlayerPath.length) {
                if (playerPositions.length > 0) {
                    setPlayerPositions(prevPositions => prevPositions.map(p => p.name === playerName ? { ...p, position: currentPlayerPath[animationStepIndex] } : p));
                } else {
                    setPlayerPositions(playersAtStart);
                }
    ...
    useEffect(() => {
        ...
        if (!turnInProgress) {  // eventually
            // console.log(currentPlayerIndex, " actually advances", numPlayers);
            setCurrentPlayerIndex((currentPlayerIndex + 1) % numPlayers);
        }
    }, [turnInProgress, numPlayers]);  //removed numPlayers
        ...
        <div className="flex flex-col items-center justify-center bg-gray-100 p-10 gap-4 space-y-0">
            ...
            <div className="flex flex-col md:flex-row items-center justify-center gap-8">
                ...
                <ChutesLaddersBoard edgeLength={edgeLength}
                    ...
                    playerPositions={playerPositions.length > 0 ? playerPositions.splice(0, numPlayers) : playersAtStart.splice(0, numPlayers)}
